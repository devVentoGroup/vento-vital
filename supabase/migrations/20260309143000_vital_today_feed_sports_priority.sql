      ...
      (
        case
          when sc.primary_sport in ('football', 'volleyball', 'taekwondo', 'basketball', 'padel') then
            case
              when b.module_key = 'training' then 12
              when b.module_key = 'recovery' then 10
              when b.module_key = 'nutrition' then 6
              when b.module_key = 'habits' then 4
              else 0
            end
          when sc.primary_sport in ('cycling', 'swimming') then
            case
              when b.module_key = 'training' then 10
              when b.module_key = 'recovery' then 12
              when b.module_key = 'nutrition' then 7
              when b.module_key = 'habits' then 5
              else 0
            end
          when sc.primary_sport = 'gym' then
            case
              when b.module_key = 'training' then 12
              when b.module_key = 'nutrition' then 8
              when b.module_key = 'recovery' then 6
              when b.module_key = 'habits' then 4
              else 0
            end
          else 0
      ...
      when s.safety_state = 'blocked' then 0
        ...
        least(
          ...
          round(
            ...
            + 0.20 * s.objective_urgency
            + 0.10 * coalesce(s.priority, 50)
            + least(12, s.sports_bonus)
          )::integer
        )
      )
