...
set client_event_id = coalesce(client_event_id, device_info ->> 'clientEventId')
where client_event_id is null
  and device_info ? 'clientEventId';

create unique index if not exists attendance_logs_employee_client_event_uidx
  on public.attendance_logs (employee_id, client_event_id)
...
drop policy if exists attendance_sync_conflicts_select_self on public.attendance_sync_conflicts;
create policy attendance_sync_conflicts_select_self
  on public.attendance_sync_conflicts
  for select
  to authenticated
  using (employee_id = auth.uid());

drop policy if exists attendance_sync_conflicts_insert_self on public.attendance_sync_conflicts;
create policy attendance_sync_conflicts_insert_self
  on public.attendance_sync_conflicts
  for insert
  to authenticated
