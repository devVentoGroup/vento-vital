...
begin
  ...
  raw_scores as (
    select
      ...
      row_number() over (partition by s.module_key order by s.task_ordering asc, s.task_instance_id)::integer as rn_in_module
    from scored s
  ),
  norm as (
    select
      r.*,
      min(r.raw_score) over () as min_raw,
      max(r.raw_score) over () as max_raw
    from raw_scores r
  )
  select
    n.task_instance_id,
    ...
    case
      when n.safety_state = 'blocked' then 0::integer
        ...
        least(
          ...
          (
            case
              when n.max_raw > n.min_raw then
                round(55 + (35 * ((n.raw_score - n.min_raw) / nullif(n.max_raw - n.min_raw, 0))))::integer
              else
                round(n.raw_score)::integer
            end
            - ((n.rn_in_module - 1) * 2)
          )
        )
