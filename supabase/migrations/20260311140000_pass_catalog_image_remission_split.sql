-- Separar imagen de venta (catálogo) e imagen de remisión en pass.catalog_items.
-- image_url = para listados/venta en app.
-- remission_image_url = para remisión (cómo debe verse en remisión).
begin;

alter table pass.catalog_items
  add column if not exists remission_image_url text;

comment on column pass.catalog_items.image_url is 'URL de la imagen para catálogo/venta (listados, app).';
comment on column pass.catalog_items.remission_image_url is 'URL de la imagen para remisión (cómo debe verse en remisión).';

-- Actualizar la función de seed para poblar ambas columnas desde products:
-- image_url <- catalog_image_url (venta)
-- remission_image_url <- image_url de products (remisión)
