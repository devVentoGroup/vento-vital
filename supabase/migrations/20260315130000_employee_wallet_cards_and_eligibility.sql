-- Carnet laboral: tabla employee_wallet_cards y RPC de elegibilidad.
-- Depende de: 20260315120000_employee_wallet_contract_and_rules.sql

-- 1. Tabla de estado del carnet laboral por empleado
create type public.employee_wallet_card_status as enum (
  'eligible',   -- cumple condiciones, puede emitir
  'issued',     -- emitido en wallet
  'revoked',    -- revocado (contrato/documentos/elegibilidad)
...
create table if not exists public.employee_wallet_cards (
  ...
  status public.employee_wallet_card_status not null default 'eligible',
  serial_number text,
  ...
  constraint employee_wallet_cards_employee_unique unique (employee_id)
);
...
create index if not exists employee_wallet_cards_employee_id on public.employee_wallet_cards (employee_id);
create index if not exists employee_wallet_cards_status on public.employee_wallet_cards (status);

alter table public.employee_wallet_cards enable row level security;

create policy employee_wallet_cards_select
...
create policy employee_wallet_cards_insert
  on public.employee_wallet_cards
  for insert
  to authenticated
  with check (
...
create policy employee_wallet_cards_update
  on public.employee_wallet_cards
  for update
  to authenticated
  using (
...
-- 2. RPC: elegibilidad para carnet wallet por empleado
-- Devuelve una fila por employee_id con contract_active, documents_complete, wallet_eligible, etc.
...
begin
  ...
  loop
    ...
    select d.id, d.issue_date, d.expiry_date
    ...
    where d.target_employee_id = v_emp_id
      ...
      and (d.expiry_date is null or d.expiry_date >= v_today)
      and d.issue_date <= v_today
    order by d.expiry_date desc nulls first
    limit 1;
    ...
    -- Si no hay reglas requeridas, considerar documentos completos
...
comment on function public.employee_wallet_eligibility(uuid) is 'Elegibilidad para carnet laboral: contrato activo, documentos requeridos y estado del carnet.';
